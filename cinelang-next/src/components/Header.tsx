...
import { Film, Search, LayoutDashboard, BookMarked, Trophy, LogOut, ChevronDown, BookOpen, Clapperboard, Layers, Brain, GalleryVertical } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center h-14 gap-4">
          ...
          <Link href="/dashboard" className="flex items-center gap-2 shrink-0">
            ...
            <span className="font-heading font-bold text-warm-900 text-base hidden sm:inline">
          ...
          <div className="flex items-center gap-1.5 text-sm">
            <select value={sourceLang} onChange={(e) => setSourceLang(e.target.value)}
              className="select text-xs py-1.5 pr-7 hidden sm:block">
              {LANGUAGES.map((l) => <option key={l.code} value={l.code}>{l.flag} {l.label}</option>)}
            </select>
            <span className="text-warm-400 text-xs font-mono hidden sm:inline">→</span>
            <select value={targetLang} onChange={(e) => setTargetLang(e.target.value)}
              className="select text-xs py-1.5 pr-7 border-cinema-200 focus:border-cinema-400">
          ...
          {/* Demo badge */}
          {demo && (
            <span className="badge bg-amber-50 text-amber-700 border-amber-200 text-xs shrink-0">Demo</span>
          )}
            ...
            return (
              <Link key={n.href} href={n.href}
                className={clsx(
                  ...
                  active ? 'bg-cinema-50 text-cinema-600' : 'text-warm-500 hover:bg-cream-200',
                )}>
